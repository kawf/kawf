...
$sql = "select mid, tid, pid, aid, state, date, subject, flags, name, email, views, DATE_FORMAT(date, \"%Y%m%d%H%i%s\") as tstamp, UNIX_TIMESTAMP(date) as unixtime from f_messages$index where tid = '" . $thread['tid'] . "' order by date";
$result = mysql_query($sql) or sql_error($sql);
...
if (isset($indexes[$index])) {
  $sql = "select mid, tid, pid, aid, state, date, subject, flags, name, email, DATE_FORMAT(date, \"%Y%m%d%H%i%s\") as tstamp from f_messages$index where tid = '" . $thread['tid'] . "' order by date";
  $result = mysql_query($sql) or sql_error($sql);
  while ($message = mysql_fetch_array($result))
    $messages[] = $message;
}
...
if (isset($user)) {
  if (isset($tthreads_by_tid[$msg['tid']])) {
    ...
    $threadlinks = "<a href=\"/track.phtml?forumname=" . $forum['shortname'] . "&tid=" . $thread['tid'] . "&page=" . $SCRIPT_NAME . $PATH_INFO . "\"><font color=\"#00d000\">tt</font></a>";
  }
} else
  $threadlinks = "";
